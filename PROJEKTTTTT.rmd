...
Test 2. H0: brak różnic pomiędzy sprzedażą a miesiacem
```{r}
...
```
Bardzo niskie p-value, więc odrzucamy H0 na rzecz H1,istnieją istotne różnice pomiędzy 
...
Przeprowadzilismy analizę danych dotyczących kawiarnii, obejmującą kilka kluczowych etapów. Na początku, dane zostały poddane transformacji, aby uzyskać spójny, kompletny zbiór gotowy do analizy. Kolejnym krokiem było wizualne przedstawienie danych za pomocą różnorodnych wykresów, koncentrując się na ilości sprzedanych produktów w poszczególnych kategoriach i jak różnego rodzaju zmienne wpływają na sprzedaż. Trzeci etap obejmował bardziej szczegółowe zanurzenie się w tematykę sprzedaży. Wykorzystane zostały podstawowe statystyki opisowe, tabele liczności, a także analiza korelacji między różnymi zmiennymi. Ostatni etap analizy skupił się na wnioskowaniu statystycznym, co pozwoliło zidentyfikować istotne cechy mające istotny wpływ na sprzedaż produktów. W rezultacie analizy ustalono, które czynniki istotnie wpływają na ilość sprzedaży. Prezentacja danych, zarówno wizualna, jak i liczbowo-statystyczna, pozwoliła na lepsze zrozumienie dynamiki sprzedaży w kawiarni oraz dostarczyła informacji pomocnych w podejmowaniu decyzji biznesowych.
Podsumowując, przeprowadzona analiza danych z zakupów w kawiarni przy użyciu programu R-Studio umożliwiła precyzyjne zidentyfikowanie zmiennych, które mają istotny wpływ na proces podejmowania decyzji dotyczących zakupu produktu.